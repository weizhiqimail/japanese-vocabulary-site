...
客户端通过同源 `fetch()` 调用 API。互不依赖的请求使用 `Promise.all()` 并行加载。例如进入学习页时，同时读取当前分类全部词汇、已掌握词汇和错题，再在客户端排除已学习 ID。
...
每个词汇生成假名、日语和翻译三种方向的题目。每题从分类词汇池生成一个正确答案和三个去重干扰项，题目、词汇和选项均再次随机。
...
文章按类别 ID 查询，正文使用 Markdown；项目文档与普通文章复用文章模块，通过类别用途区分。
...
4. 写入模式在单个事务中新增词汇并补分类关联。
5. 提交后重新执行完整来源比较，缺失或缺标签不为 0 时任务失败。
...
学习进度、收藏组和收藏按用户键隔离；词汇、类别和文章是共享内容。邮箱请求头必须由可信托管代理注入。若未来开放独立访问，需要加入正式认证和角色权限。
