...
动态路由使用允许列表拒绝未知模块。客户端通过 `usePathname()` 解析当前页面，通过 `useSearchParams()` 读取查询条件，使用 `router.push()` 或 `router.replace()` 同步 URL。复习和管理子模块具有独立路径，因此支持刷新、前进后退和直接访问。

学习、复习和词库将 `categoryId`、`pageNum`、`pageSize` 写入查询参数；收藏页还写入 `groupId`。路由查询参数是可分享、可恢复状态，组件状态负责即时交互。浏览器刷新或历史导航后，组件从查询参数重新恢复筛选和分页。
...
分类属于各页面自己的筛选条件，不使用全局分类状态控制所有页面。复习数据在客户端按所选类别或收藏组载入后分页，词库分页仍由 API 服务端执行。
...
普通模式用三个布尔状态统一控制日语、假名、翻译。默记模式用 `{vocabularyId}:{field}` 键保存每张卡片的单独切换状态，默认显示日语、隐藏假名和翻译。学习页、收藏本与词库页各自维护状态，互不干扰。
...
- 桌面词库使用完整表格；手机端复用表格数据但渲染为两层卡片：首层为“词汇＋假名 / 收藏”，次层为“翻译＋标签”，并隐藏管理操作列。
- 复习、管理在桌面端左导航右内容，手机端上导航下内容。
- 手机端品牌区把版本号和构建时间分为两行；主导航常驻首页、学习和导航入口，其余页面与 GitHub 仓库入口收纳在遮罩浮层。
...
顶部版本信息由 Vite `define` 在构建时注入：版本号在发布前递增，构建时间使用 `Asia/Tokyo` 时区生成。客户端只读取编译后的常量，不额外发起 HTTP 请求。桌面端版本信息位于品牌右侧，手机端位于品牌名称下方并分成两行。
...
`demo/` 是独立于正式站点的 Vite + React 纯前端工程，用于验证 `docs/PRD-V2.md` 的页面结构和交互。它不连接正式 API、在线 MySQL 或 Sites 运行环境，所有数据均为写死的内存模拟数据。

### 12.1 技术与边界

- React、Vite、Bootstrap、Bootstrap Icons 和 SCSS。
- 使用浏览器 History 路由，不使用 Hash；分页、筛选参数保存在普通 URL 查询参数中。
- Vite 开发服务器使用 SPA 回退；`demo/public/_redirects` 随构建输出，为兼容平台把未知路径重写到 `/index.html`。其他 Web 服务器部署 Demo 时必须配置等价回退，保证刷新二级路由不返回 404。
- 拥有独立 `package.json`、依赖锁文件、构建产物和运行命令，不修改正式站点入口。
- 页面行为只用于需求评审，不代表 V2 后台 API 或数据库已经实现。
- Demo 使用版本化 `localStorage` 作为模拟数据库；初始化数据和后续 CRUD、关联、收藏、掌握、审核、测试记录及设置统一持久化。该存储方案不得用于正式系统。
...
- 错题本、掌握记录、收藏本。
- 词汇、语法和混合测试。
- 正式词库、词汇详情、语法库、语法详情、句子库、文章占位页。
- 词汇、语法、句子详情通过稳定 ID 路由互相跳转，关系在本地模拟数据库中双向维护。
- 管理端 AI 导入审核、标签、词性和设置。

### 12.3 验证

在 `demo/` 中运行 `npm run build`。Demo 的修改不替代正式站点构建与测试；正式功能开发仍需按仓库主流程验证。
