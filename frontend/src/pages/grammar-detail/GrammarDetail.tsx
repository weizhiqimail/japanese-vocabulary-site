...
import { deleteGrammar, getGrammar } from '@/http/api/grammars.api';
...
export function GrammarDetail() {
  ...
  const load = useCallback(async () => {
    setBusy(true);
    try {
      setItem(await getGrammar(grammarId));
    } finally {
      setBusy(false);
    }
  }, [grammarId]);
  ...
  const remove = async () => {
    if (!window.confirm('确认逻辑删除这个语法吗？')) return;
    setBusy(true);
    try {
      await deleteGrammar(grammarId);
      navigate('/w/words/grammars');
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <VStack align="stretch" spacing={6}>
      ...
      >
        <Box>
          <Text color="slate.500">语法详情</Text>
          <Heading size="xl" mt={2}>
            {String(item?.pattern || '')}
          </Heading>
        </Box>
        <HStack wrap="wrap">
          <Button onClick={sentenceCreator.onOpen}>新增例句</Button>
