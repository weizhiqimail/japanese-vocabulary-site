...
export async function GET(request: Request) {
  ...
  const params = search ? [categoryId, like, like, like] : [categoryId];
  ...
  try {
    const [rows] = await db.query(
      `SELECT v.id, v.word, v.reading, v.meaning,
       ...
       JOIN categories c_filter ON c_filter.id = vcl_filter.category_id AND c_filter.id = ?
    ...
    const [countRows] = await db.query(
      `SELECT COUNT(DISTINCT v.id) AS total
       ...
       JOIN categories c ON c.id = vcl.category_id AND c.id = ?
       WHERE 1 = 1 ${searchSql}`,
      params,
    );
    const items = (rows as Array<Record<string, unknown>>).map((row) => ({
      ...
      categoryIds: String(row.categoryIdList || "").split(",").filter(Boolean).map(Number),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if ((categoryRows as unknown[]).length !== categoryIds.length) {
